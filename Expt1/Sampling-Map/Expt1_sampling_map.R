...
setwd("~/Library/CloudStorage/OneDrive-UniversityofNewHampshire/GreenManureProject/Expt1")

#making custom df
locations <- c("Durham Reservoir (DR)",
               "LaRoche Pond (LR)",
               "Mill Pond (M)",
               "Thompson Farm (TF)",
               "Upper Mill Pond (UM)",
...
regular_locations <- sampling_locations %>%
    filter(!(Location %in% c("Kingman Farm", "Organic Dairy Research Farm", "UNH Durham Campus")))
