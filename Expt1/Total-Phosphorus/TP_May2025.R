...
mod_microH <- MCMCglmm(ppb ~  -1 + cyano:geno, data=tp_microH, verbose=F, nitt = 101000, thin = 10, burnin = 1000 )
summary(mod_microH)
...
mod_noHmicro <- MCMCglmm(ppb ~  -1 + cyano:micro, data=tpdat_noHmicro, verbose=F, nitt = 101000, thin = 10, burnin = 1000 )
...
TPplot_microH <- tp_microH |> 
    ...
    stat_summary(fun = mean, geom = "point", shape = 16, size = 2.5, position = position_dodge(width = 0.6)) + 
    stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = .35, size = .5, position = position_dodge(width = 0.6)) +
    scale_color_manual(values = c("N" = "black", "Y" = "aquamarine4")) +
    scale_x_discrete(labels = geno_labels) +
    ylim(0, 50000) +
    labs(title = "Home Microbiome by Genotype", x = "Duckweed Genotype", y = "Total Phosphorus (µg/L)") +  
    theme_classic() +
    theme(
    ...
    geom_hline(yintercept = 45570, linetype = "dashed", color = "red", size = 1, show.legend = TRUE) +
    annotate(
        "text",
        x = 1, y = 45570 + 1500,
        label = "Initial TP (45,570 µg/L)",
        color = "red", size = 2, hjust = 0.2, vjust = 0)
TPplot_microH
...
TPplot_others <- tpdat_noHmicro |> 
    ...
    stat_summary(fun = mean, geom = "point", shape = 16, size = 2.5, position = position_dodge(width = 0.6)) + 
    stat_summary(fun.data = mean_sdl, fun.args = list(mult = 1), geom = "errorbar", width = 0.3, size = .5, position = position_dodge(width = 0.6)) +
    scale_color_manual(values = c("N" = "black", "Y" = "aquamarine4"), name = "Cyanobacteria") + 
    scale_x_discrete(labels = micro_labels) +
    ylim(0, 50000) +
    labs(title = "Other Microbiomes, Genotypes Combined", x = "Microbiome", y = "") +  
    theme_classic() +
    theme(
        legend.position = "none",
        axis.title.x = element_text(size = 7, margin = margin(t = 8)),
        ...
        axis.line.y = element_line(colour = "black", linetype = "dashed", size = .5),
...
TP_variance_plot <- ggplot(TPvariance_data, aes(x = "", y = Variance, fill = Factor)) +
...
TPmicro_combined_plot <- plot_grid(TPplot_microH, TPplot_others, TP_variance_plot, nrow = 1, rel_widths = c(1.5,1,.65))
