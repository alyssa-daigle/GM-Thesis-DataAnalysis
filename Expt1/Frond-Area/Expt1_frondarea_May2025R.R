...
setwd("~/Library/CloudStorage/OneDrive-UniversityofNewHampshire/GreenManureProject/Expt1/expt1_dataanalysis/Growth")
...
mod <- MCMCglmm(RGR ~ -1 + cyano:micro, data=dat, verbose=F, nitt = 11000, thin = 10, burnin = 100 )
summary(mod)
...
growth_variance_data <- data.frame(
    ...
    Variance = c(growth_variance_cyano, growth_variance_geno, growth_variance_micro)
)

# variation explained plot
growth_variance_plot <- ggplot(growth_variance_data, aes(x = "", y = Variance, fill = Factor)) +
...
growth_combined_plot <- plot_grid(growth_plot, growth_variance_plot, nrow = 1, rel_widths = c(5,2))

growth_combined_plot

ggsave("Expt1_growth_plot.jpg", growth_combined_plot, width = 6, height =3.5, dpi = 500)
ggsave("~/Library/CloudStorage/OneDrive-UniversityofNewHampshire/GreenManureProject/WRITING/plots/Expt1_growth_plot.jpg", growth_combined_plot, width = 6, height = 3.5, dpi = 500)