...
setwd("~/Library/CloudStorage/OneDrive-UniversityofNewHampshire/GreenManureProject/Expt1/expt1_dataanalysis/ELISA")
...
ELISA_dat_filtered <- ELISA_dat %>%
    filter(!is.na(cyano)) %>%
    filter(!(sample == "25_c2" & treatment == "M_N_N") & !(sample == "26_b3" & treatment == "W_Y_N"))

#linear modeling
ELISA_glmm <- MCMCglmm(MC ~ -1 + cyano:micro, data=ELISA_dat_filtered, verbose=F, nitt=101000, thin=10, burnin=1000) 
summary(ELISA_glmm)
...
ELISA_dat_filtered$micro <- factor(ELISA_dat_filtered$micro, levels = c("N", "H", "KF", "ODR"))
...
MCplot <- ggplot(ELISA_dat_filtered, aes(x = micro, y = MC, color = cyano)) +
    ...
    scale_x_discrete(labels = c("N" = "Uninoculated", "H" = "Home", "KF" = "Kingman Farm", "ODR" = "Dairy Farm")) +
...
#stacked bar plot
MC_variance_plot <- ggplot(MC_variance_data, aes(x = "", y = Variance, fill = Factor)) +
...
MC_combined_plot <- plot_grid(MCplot, MC_variance_plot, nrow = 1, rel_widths = c(5,2))

MC_combined_plot

ggsave2("Expt1_MC_plot.jpg", plot = MC_combined_plot, width = 6, height =3.5, dpi = 500)
ggsave("~/Library/CloudStorage/OneDrive-UniversityofNewHampshire/GreenManureProject/WRITING/plots/Expt1_MC_plot.jpg", MC_combined_plot, width = 6, height = 3.5, dpi = 500)
...
MC_line_plot <- ELISA_dat |> 
    ...
    ggplot(aes(x = cyano, y = mean_conc, fill = geno, color = geno, group = geno)) +
    ...
    geom_errorbar(aes(ymin = mean_conc - se_conc, ymax = mean_conc + se_conc),
                  width = 0.05, size = .75) + 
    labs(x = "*M. aeruginosa* Spike", y = "Average Microcystin (µg/g Duckweed)", color = "Duckweed\nGenotype") +
...
ggsave("Expt1_MC_line_plot.jpg", plot = MC_line_plot, width = 4, height = 3, dpi = 300)
...
micro_mod <- MCMCglmm(MC ~ -1 + micro:cyano, data=ELISA_dat_filtered, verbose=F, nitt=101000, thin=10, burnin=1000)
summary(micro_mod)

micro_labels <- c("H" = "Home", 
                  "ODR" = "Dairy \nFarm", 
                  "N" = "None", 
                  "KF" = "Kingman \nFarm")
...
MCplot_micro <- ggplot(ELISA_dat_filtered, aes(x = micro, y = MC, color = cyano)) +
    ...
    scale_color_manual(values = c("N" = "black", "Y" = "aquamarine4"),
                       labels = c("N" = "No", "Y" = "Yes")) +
    labs(x = "Microbiome Source", color = "*M. aeruginosa* <br> Spike", y = "Microcystin (µg/g Duckweed)") +
...
ggsave("Expt1_MC_micro_plot.jpg", plot = MCplot_micro, width = 5, height = 3, dpi = 500)
